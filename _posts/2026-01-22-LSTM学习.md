...
回顾RNN的结构
![alt text](LSTM_img/RNN.png)

Long short-term memory结构
![alt text](LSTM_img/RNN.png)
We can see that 同等数量units 参数量是RNN的4倍(一个很重要的点)

LSTM网络则是将每一个隐藏层单元替换为一个LSTM unit。
多层LSTM网络结构如下：除了输入序列x外，隐藏层输出h，使用Extension “peephole”，memory cell输出也作为下一时刻的输入
![alt text](LSTM_img/multiLayerLSTM.png)

### 训练
BPTT(backpropagation through time)，考虑时间信息
![alt text](LSTM_img/BPTT.png)

RNN traning比较困难，容易梯度爆炸
![alt text](LSTM_img/gradient_explosion.png)
